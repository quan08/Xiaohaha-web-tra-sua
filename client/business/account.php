...
function login()
{
    ...
    if ($loginToken != "") {
        ...
        if ($user['role'] == 1) {
            unset($user['password']);
            $_SESSION['auth'] = $user;
            header('location:' . BASE_URL . 'tai-khoan/guest');
            die;
        } else if ($user['role'] == 2) {
            unset($user['password']);
            $_SESSION['auth'] = $user;
            header('location:' . BASE_URL . 'tai-khoan/staff');
            die;
        } else if ($user['role'] == 5) {
            unset($user['password']);
            $_SESSION['auth'] = $user;
...
function guest()
{
    client_render('account/check/guest.php');
}
function staff()
{
    client_render('account/check/staff.php');
}
// 